...
extern "C" void port_drain_pending_display_list(void)
{
	...
	/* Count this drain as a frame's gfx submission. PortPushFrame uses
	 * sDLSubmitsThisFrame to drive the VI-idle present path so the
	 * displayed framebuffer keeps updating even when the game skipped
	 * the gfx task this VI period (real-N64 parity). */
	sDLSubmitsThisFrame++;
	gbi_trace_end_frame();
