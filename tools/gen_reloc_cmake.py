...
def main() -> int:
    ...
    ap.add_argument("--cmake-source-prefix",
                    default="${CMAKE_SOURCE_DIR}/decomp/src/relocData",
                    help="CMake-variable expression prefixed to each emitted "
                         "source path. Default keeps in-tree compatibility; "
                         "callers using the toolkit as a submodule should "
                         "pass '${RELOCDATA_DECOMP_DIR}/src/relocData' so the "
                         "generated include resolves against whatever the "
                         "consuming project sets RELOCDATA_DECOMP_DIR to.")
    ...
    # Force utf-8 — comments contain U+2192 RIGHTWARDS ARROW and the
    # Windows default (cp1252) can't encode it, breaking the generator
    # silently when run from a non-UTF-8 locale.
    with args.output.open("w", encoding="utf-8") as f:
        ...
        f.write("# Source-compiled (real .c → cc → chain-encode):\n")
        for fid, basename, res_path in eligible:
            f.write(
                f'add_reloc_resource({fid} '
                f'{args.cmake_source_prefix}/{basename} '
                f'"{res_path}")\n'
            )
